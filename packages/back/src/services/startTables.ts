...
export function startTables(): GameData {
  const state = updateGameState((data) => {
    ...
    data.darkAvengersThreatIni =
      DARK_AVENGERS_THREAT_INI_PER_PLAYER * numPlayers;
    data.darkAvengersThreatMax =
      DARK_AVENGERS_THREAT_MAX_PER_PLAYER * numPlayers;
    data.ironPatriotLife =
      IRON_PATRIOT_LIFE_PER_PLAYER * normal +
      IRON_PATRIOT_LIFE_EXP_PER_PLAYER * expert;
